...
try:
    ...
    sys.exit(f"ttyscan2: unsupported platform (missing required module: {exc})")

try:
    _TTYSCAN_QUERY_TIMEOUT = float(os.environ.get('TTYSCAN_QUERY_TIMEOUT', '1.0'))
except ValueError:
    print(f"ttyscan2: TTYSCAN_QUERY_TIMEOUT value "
...
def warn(msg):
    print(f"ttyscan2: {msg}", file=sys.stderr)
        ...
        if cur == '\\' and idx + 1 < len(value):
            ...
            if nxt in '01234567':
                ...
                try:
                    ...
                    warn(f"invalid octal escape in terminfo value: {value[idx:end]!r}")
                    result.append(cur)
                idx = end
                continue
